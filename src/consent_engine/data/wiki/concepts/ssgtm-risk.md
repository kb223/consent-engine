...
sGTM containers process tracking calls on a server rather than in the browser. This creates a consent bypass risk:

**Client-side JavaScript cannot block a server-to-server call.**

If a sGTM container is configured to forward advertising tags regardless of consent state:
...
## How to Identify sGTM in an Audit

**Signals suggesting sGTM is present:**
...
## Distinguishing Compliant vs Non-Compliant sGTM

| Signal | Implication |
|---|---|
| `gcs=` parameter present in first-party requests during denied-consent scan | sGTM is passing Consent Mode signals — likely compliant |
| No `gcs=` parameter despite CMP being present | sGTM may not be passing consent signals — high risk |
| Conversion events fire from first-party domain after opt-out | Non-compliant sGTM bypass |
| Container follows Google Tag Gateway pattern (see [[technical/google-tag-gateway]]) | May be compliant GTG, not rogue sGTM |

## GPC and sGTM

The `Sec-GPC: 1` header is an HTTP request header. When a browser sends this to a first-party server endpoint (sGTM), the server receives it — but many sGTM configurations do not:
1. Read the GPC header
2. Map it to consent state
3. Block downstream advertising calls accordingly

This is a systemic implementation gap. Even a site with a fully compliant client-side CMP may be violating GPC via sGTM.

## Legal Liability

Regulators are beginning to examine sGTM specifically as a consent bypass mechanism. The 2024–2026 enforcement trend confirms:
...
Google requires all sGTM implementations to:
